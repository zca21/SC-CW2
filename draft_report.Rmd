...
\begin{equation*}
Pr(U>\boldsymbol{b})=\prod_{i=1}^{d}Pr\left(Z>\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}}\right)=\prod_{i=1}^{d}\int_{s_{i}}^{\infty}\phi(x)dx=\prod_{i=1}^{d}\int_{-\infty}^{\infty}\frac{\phi(x)}{q_{i}(x)}q_{i}(x)I(x>s_{i})dx = E\left[\prod_{i=1}^{d} \frac{\phi(V_{i})}{q_{i}(V_{i})}I(V_{i}>s_{i})\right]
\end{equation*}

With $V_{i}, i \in \{1,..,d\}$ i.i.d. random variables from the proposal distribution, $q_{i}$ the density of the proposal distribution, $\phi$ the density of a standard normal distribution and $I$ the indicator function. 
...
Where $w_{i}^{(j)}$ is $\frac{\phi(V_{i})}{q_{i}(V_{i})}$ on the jth iteration and the superscript $(j)$ denoting the jth sampling. Therefore, to not waste any sample values let the proposal distributions be exponential distributions shifted by $s_{i}$ as all sample values (denoted $v_{i}$) drawn from these proposal distributions will satisfy $v_{i}>s_{i}$ for $i \in \{1,...,d\}$. Thus, all values generated from these proposal distributions can be used to calculate $I_{n}^{IS}$ from equation (4) as the indicator function is 1 for each $v_{i}$ drawn from each $V_{i}$. 
...
Next for each iteration j we must simulate the random walk $U$ using equation (3), the initial value $u_{0}$ and times $t_{i}, i \in \{0,..,d\}$ and storing the values of the random walk $\boldsymbol{u}$ as mentioned in part a. Using these values and the relevant proposal distribution of an exponential distribution shifted by $s_{i}$ (described in part a) to generate $v_{i}, i \in \{1,..,d\}$.
...
Then calculate the product of the weights $\prod_{i=1}^{n}w_{i}$ and save this as the jth value in a vector.

#### Calculating probability:
To calculate $p_{tail}(\boldsymbol{b})$, the previous 2 steps are performed n times thus we have n values of the product of the weights (which I will denote $\hat{w}^{(j)}$ where the value of $j$ denotes the jth iteration) which we then use to calculate the importance sampling (IS) estimator using equation (4) in part a:

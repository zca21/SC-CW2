...
For acceptance-rejection sampling (AR sampling) we need to find a constant $M>0$ such that $\pi(x) \leq Mg(x), \forall x \in \mathbb{R}$ for $\pi(x)$ the target distribution and $g(x)$ the proposal density defined in equations 1 and 2 above. This constant M determines the acceptance probability which is $\frac{1}{M}$ and hence the efficiency of the algorithm. For multiple possible values of $\lambda$ in $g(x)$ the value of $\lambda$ that results in a smaller constant M will have a higher acceptance probability and thus will be our preferred value of $\lambda$ for the algorithm. To find M for each $\lambda$ value I split the real domain of $x$ into 3 regions (see plot 1) that I call 1,2 and 3 which correspond to the different expressions of $\pi(x)$ depending on the value of $x$. Then study the relationship between $g(x)$ and $\pi(x)$ within these 3 regions to find $M_{1},M_{2},M_{3}$ which correspond to the smallest constant such that $\pi(x) \leq M_{i}g(x)$ for the ith region. Then set $M=max(M_{1},M_{2},M_{3})$ as this is the smallest constant that satisfies $\pi(x) \leq Mg(x)$ for all possible values of $x \in \mathbb{R}$.
...
#### Region 2:
For the second region [$s-b,s$], $\pi(x)=\frac{1}{2b}$. Considering that $g(x)$ is an increasing function in this region (see plot 1) it will be at it's minimum value at $x=s-b$ and the value of $\pi(x)$ is constant. Thus, setting $M_{2}=\frac{\pi(s-b)}{g(s-b)}=\frac{exp(\lambda|s-2b|)}{\lambda b}$ will satisfy the equation $\pi(x) \leq M_{2}g(x)$ for all values of  $x$ in this region.
...
## Part b:
To implement A-R sampling using the proposal density with $\lambda=0.5$ and thus $M=3.2e^{0.875}$ to obtain samples from $\pi(x)$ with parameters $s$, $a$ and $b$ I use the following algorithm:

#### Sampling from the proposal distribution:
To sample from $g(x)$, use the inverse transformation technique by drawing $u$ from a Uniform(0,1) distribution then if $u<0.5$ set $x=b+\frac{log(2u)}{\lambda}$, else set $x=b-\frac{2-2u}{\lambda}$. To obtain the value $x$, drawn from $g(x)$ the proposal distribution. 
...
\end{equation*}
For $Z$ the standard normal and $u_{i-1}$ a value drawn from $U_{i-1}$. Therefore, to find $Pr(U_{i}>b_{i})$ we can find the probability a standard normal distribution is greater than $\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}}$. Therefore, letting $\hat{p}_{t}^{(i)}$ be the importance sampling estimator of $p_{t}^{(i)}=Pr(U_{i}>b_{i})$ we can use the following equation: 

\begin{equation*}
 \hat{p}_{t}^{(i)} = \frac{1}{n}\sum_{j=1}^{n}\frac{\phi(V_{j})}{q_{i}(V_{j})}I\left(V_{j}>\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}} \right)
\end{equation*}

With $V_{j}, j \in \{1,..,n\}$ iid random variables from the proposal distribution $q_{i}$, $\phi$ the density of a standard normal distribution and $I$ the indicator function.  Therefore, to not waste any sample values let $q_{i}$ be an exponential distribution shifted by $s_{i}$ where:

\begin{equation*}
 s_{i}=\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}} 
\end{equation*}

As all sample values drawn from these proposal distributions $\boldsymbol{q}=\{q_{1},....,q_{d}\}$ will satisfy $v_{i}>\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}}$ for $i \in \{1,...,d\}$. Thus, all values generated from these proposal distributions can be used to calculate $\hat{p}_{t}^{(i)}$ for $i \in \{1,..,d\}$ (as the indicator function is 1 for all samples drawn to calculate each $\hat{p}_{t}^{(i)}$). 

To calculate $q_{i}$ however, we need to know the value $u_{i-1}$ (a value drawn from the random variable $U_{i-1}$) for each $i$ . Thus, each iteration will need to generate $\boldsymbol{u} = (u_{1},...,u_{d-1})$ a realization of the random walk sequence $U=(U_{i})_{i \in \{1,...,d-1\}}$ conditioned on $U_{0}=u_{0}$ using equation (3) with the initial value $u_{0}$ of the random walk and times $t_{i}$,  $i \in \{0,...,d\}$. Then storing these values to be accessed to generate a sample from the proposal distribution.

Thus, using $\boldsymbol{q}$ we can generate generate the sample vector $\boldsymbol{v}=\{v_{1},....,v_{d}\}$ where each $v_{i}$ satisfies $v_{i}>\frac{b_{i}-u_{i-1}}{\sqrt{t_{i}-t_{i-1}}}$. Thus, as $\prod_{i=1}^{d}\hat{p}_{t}^{(i)}=p_{tail}(\boldsymbol{b})$ each vector is used to calculate $p_{tail}(\boldsymbol{b})=p_{tail}(\boldsymbol{a})$. 
...
Next for each iteration j we must simulate the random walk $U$ using equation 3 and storing the realization $\boldsymbol{u}$ as mentioned in part a. Using these values and the relevant proposal distribution $q_{i}$ (described in part a) to generate $v_{i}$ a sample from $q_{i}$ for each step $i$ of the random walk sequence $U$.

#### Calculating weights:
Next calculate weights $w_{i}$ of each $v_{i}$ using the formula $w_{i}=\frac{\phi(v_{i})}{q_{i}(v_{i})})$ where $\phi$ is the demnsity of the standard normal and $q_{i}$ is the density of the proposal distribution, an exponential distribution with rate $\lambda$ and shifted by $s_{i}$ as defined in part a. 

Then the combined weight used to calculate $p_{tail}(\boldsymbol{b})$ is the product of the weights $\prod_{i=1}^{n}w_{i}$. This is the weight used to calculate $Pr(\boldsymbol{U}>\boldsymbol{b}|U_{0}=u_{0})$ (using the fact that $w_{i}$ is weight of $Pr(U_{i}>b_{i}|U_{0}=u_{0})$).

#### Calculating probability:
To calculate $p_{tail}(\boldsymbol{b})$, The previous 2 steps are performed n times thus we have n values of the combined weight (which I will denote $\hat{w}$) which we then use to calculate the importance sampling (IS) estimator defined as:
...
For each iteration j start by drawing $\boldsymbol{P}_{\boldsymbol{T}}^{new}$ the proposed value of $\boldsymbol{P}_{\boldsymbol{T}}$ from the proposal density by drawing each $p_{x}'$ with $x \in \{s,c,r\}$ from a Dirichlet $(20p_{x}^{(cur)})$ distribution and combining these to get $\boldsymbol{P}_{\boldsymbol{T}}^{new}=(p_{s}',p_{c}',p_{r}')^{T}$.
...
To calculate the RHS we use the Markov property and equation 8 and 9 in the coursework question sheet to get following equation:
